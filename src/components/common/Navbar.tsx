...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-black/95 backdrop-blur-sm border-b border-gray-800 shadow-lg">
          ...
          <div className="hidden md:flex space-x-1">
            {navLinks.map((link) => (
              <Link
                key={link.path}
                to={link.path}
                className={`flex items-center gap-1.5 px-3 py-2 rounded-md transition hover:bg-gray-800 ${
          ...
          <div className="container mx-auto px-4 py-3 space-y-1">
            {navLinks.map((link) => (
              <Link
                key={link.path}
                to={link.path}
                className={`flex items-center gap-2 px-4 py-3 rounded-md transition hover:bg-gray-800 ${
                  location.pathname === link.path 
                    ? 'text-purple-400 font-medium' 
                    : 'text-gray-300'
                }`}
