...
// Import the Movie interface to fix type errors
...
export const searchByYearRange = async (yearFrom: number, yearTo: number): Promise<Movie[]> => {
  try {
    // Search for both movies and TV shows in the specified year range
    const [movieResponse, tvResponse] = await Promise.all([
      fetch(`${TMDB_BASE_URL}/discover/movie?api_key=${TMDB_API_KEY}&language=fr-FR&primary_release_date.gte=${yearFrom}-01-01&primary_release_date.lte=${yearTo}-12-31&sort_by=popularity.desc`),
      fetch(`${TMDB_BASE_URL}/discover/tv?api_key=${TMDB_API_KEY}&language=fr-FR&first_air_date.gte=${yearFrom}-01-01&first_air_date.lte=${yearTo}-12-31&sort_by=popularity.desc`)
    ]);
    
    if (!movieResponse.ok || !tvResponse.ok) {
      throw new Error('Failed to search by year range');
    }
